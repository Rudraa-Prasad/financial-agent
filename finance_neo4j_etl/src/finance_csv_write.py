...
def load_finance_graph_from_csv() -> None:
    ...
    with driver.session(database="neo4j") as session:
        ...
        employee_name: employees.employee_name, position: employees.position, 
        year_of_joining: toInteger(employees.year_of_joining), salary: toFloat(employees.salary)}});
        """
        _ = session.run(query, {})
    ...
    with driver.session(database="neo4j") as session:
        ...
        MERGE (t:Transaction {{id: toInteger(transactions.transaction_id), transaction_type: transactions.transaction_type, amount: toFloat(transactions.amount), date: datetime(replace(transactions.date, " ", "T"))}});
        """
        _ = session.run(query, {})
